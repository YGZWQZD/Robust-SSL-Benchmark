...
fig.suptitle("Agnews with Inconsistent Label Spaces", fontsize=13)
...
for dataset in datasets:
    f = open('./Class/'+dataset+'_class.csv', "r",encoding="utf-8")
    g=open('./Class/'+dataset+'_stat.csv', "w",encoding="utf-8")
...
#         dict_std[row[0]+str(row[1])+str(row[2])+str(row[3])] = float(row[5])    
    ...
    axes.set_xlabel("Label Space Inconsistency t", fontdict={"size": 8})
    ...
    pos+=1
plt.tight_layout()
plt.savefig('Label_agnews.eps',dpi=300)
plt.savefig('Label_agnews.png',dpi=300)
plt.show()
