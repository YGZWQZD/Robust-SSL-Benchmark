...
map_d={'c':'Caltech','i':'ImageNet','p':'Pascal'}
...
fig.suptitle("Image-CLEF"+" with "+str(100)+" labels", fontsize=13)
# plt.title("Office-31"+"with "+100+" labels", fontdict={"size": 26})
# plt.xlabel("Class Inconsistency", fontdict={"size": 26})

f = open('./Distribution/Image_distribution.csv', "r",encoding="utf-8")
g=open('./Distribution/Image_distribution_stat.csv', "w",encoding="utf-8")
...
for source in domain:
    for target in domain:
        ...
        for method in methods:
            ...
            y_std=y_std*100/2
            X=rate_list_f
            axes[i,j].plot(X, y, symbol, label=reveal, color=color)

        print(i)
        print(j)
        axes[i,j].legend(loc="lower left")
        axes[i,j].set_xlabel("Data Distribution Inconsistency Rate t", fontdict={"size": 8})
        axes[i,j].set_ylabel("Acc_T(t) (%)", fontdict={"size": 8})
        axes[i,j].grid()
        axes[i,j].set_xticks(rate_list_f)
        axes[i,j].set_yticks([min_mean*100+(max_mean-min_mean)*i*10 for i in range(0,11,1)])
        axes[i,j].set_title(map_d[source]+'/'+map_d[target],fontdict={"size": 8})
        # axes[i,j].savefig('iris' + '_deep' + '_class' + '_labels_' + str(labels)+'.png' , dpi=300)
        # axes[i,j].show()
        pos+=1
plt.tight_layout()
plt.savefig('Distribution_Image-CLEF.eps',dpi=300)
plt.savefig('Distribution_Image-CLEF.png',dpi=300)
plt.show()