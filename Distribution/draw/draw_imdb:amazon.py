...
fig.suptitle("IMDB/Amazon with Inconsistent Data distributions", fontsize=13)
...
for dataset in datasets:
    f = open('./Distribution/'+dataset+'_distribution.csv', "r",encoding="utf-8")
    g=open('./Distribution/'+dataset+'_stat.csv', "w",encoding="utf-8")
...
#         dict_std[row[0]+str(row[1])+str(row[2])+str(row[3])] = float(row[5])    
    ...
    for method in methods:
        ...
        y_std=y_std*100/2
        X=rate_list_f
        axes.plot(X, y, symbol, label=reveal, color=color)
        d['dataset']=dataset
        d['algorithm']=method
    ...
    axes.set_xlabel("Data Distribution Inconsistency t", fontdict={"size": 8})
    axes.set_ylabel("$Acc_T(t)(\%)$", fontdict={"size": 8})
    axes.grid()
    axes.set_xticks(rate_list_f)
    axes.set_yticks([min_mean*100+(max_mean-min_mean)*i*10 for i in range(-5,11,1)])
    axes.set_title(dataset,fontdict={"size": 8})
    # axes[i,j].savefig('iris' + '_deep' + '_class' + '_labels_' + str(labels)+'.png' , dpi=300)
    # axes[i,j].show()
    pos+=1
plt.tight_layout()
plt.savefig('Distribution_IMDB_Amazon.eps',dpi=300)
plt.savefig('Distribution_IMDB_Amazon.png',dpi=300)
plt.show()