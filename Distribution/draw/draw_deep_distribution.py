...
g=open('./Distribution/'+'deep_tabular'+str(labels)+'_stat.csv', "w",encoding="utf-8")
...
for dataset in datasets:
    f = open("Distribution/"+dataset+"_deep_distribution_labels_"+str(labels)+'.csv', "r",encoding="utf-8")
    f_read=csv.reader(f)
    dict_mean={}
    dict_std={}
    for row in f_read:
    ...
    pos+=1
plt.tight_layout()
plt.savefig('Distribution_deep_'+'tabular'+'_labels_'+str(labels)+'.eps',dpi=300)
plt.savefig('Distribution_deep_'+'tabular'+'_labels_'+str(labels)+'.png',dpi=300)
plt.show()
