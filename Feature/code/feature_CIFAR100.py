...
f=open("CIFAR100"+'_feature'+'_labels_'+str(labels)+'_'+str(iteration)+'_7.csv', "w", encoding="utf-8")
...
for source in ['cifar100']:
    for target in ['cifar100']:
        if dataset == 'Office-31':
            ...
            num_classes = 12
        dataset=CIFAR100(root='/lamda/jialh/Work/LAMDA-SSL_NJU/lamda-ssl/LAMDA-SSL/Download/cifar-100-python',labeled_size=None,stratified=False,shuffle=False,download=True,default_transforms=True)
        num_classes=100
        '''
        if source==target:
            continue
