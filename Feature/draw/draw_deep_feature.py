...
g=open('./Feature/'+'deep_tabular'+str(labels)+'_stat.csv', "w",encoding="utf-8")
...
for dataset in datasets:
    f = open("Feature/"+dataset+"_deep_feature_labels_"+str(labels)+'.csv', "r",encoding="utf-8")
    f_read=csv.reader(f)
    dict_mean={}
    dict_std={}
    for row in f_read:
    ...
    axes[j].set_xlabel("Feature Space t", fontdict={"size": 8})
    ...
    pos+=1
plt.tight_layout()
plt.savefig('Feature_deep_'+'tabular'+'_labels_'+str(labels)+'.eps',dpi=300)
plt.savefig('Feature_deep_'+'tabular'+'_labels_'+str(labels)+'.png',dpi=300)
plt.show()
