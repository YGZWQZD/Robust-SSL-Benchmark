...
g=open('./Feature/'+'statistical_tabular'+str(labels)+'_stat.csv', "w",encoding="utf-8")
...
for dataset in datasets:
    f = open("./Feature/"+dataset+"_statistical_feature_labels_"+str(labels)+'.csv', "r",encoding="utf-8")
    f_read=csv.reader(f)
    dict_mean={}
    dict_std={}
    for row in f_read:
    ...
    pos+=1
plt.tight_layout()
plt.savefig('Feature_statistical_'+'tabular'+'_labels_'+str(labels)+'.eps',dpi=300)
plt.savefig('Feature_statistical_'+'tabular'+'_labels_'+str(labels)+'.png',dpi=300)
plt.show()
