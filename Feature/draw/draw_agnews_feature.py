...
fig.suptitle("Agnews with Inconsistent Feature Spaces", fontsize=13)
...
#         dict_std[row[0]+str(row[1])+str(row[2])+str(row[3])] = float(row[5])    
    ...
    axes.set_xlabel("Feature Space Inconsistency t", fontdict={"size": 8})
    ...
    pos+=1
plt.tight_layout()
plt.savefig('Feature_agnews.eps',dpi=300)
plt.savefig('Feature_agnews.png',dpi=300)
plt.show()
